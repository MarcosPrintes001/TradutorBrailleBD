...
cv2.waitKey(0)
"""
canvas = np.ones((100, 100, 3)) * 255 #imagem 400x300, com fundo branco e 3 canais para as cores
...
for i in range(0,len(index_lin)-3,3): # VLine
    for j in range(0,len(index_col)-2,2): # VCol
        sub_imagem(imageSeg, index_lin[i],index_lin[i+3], index_col[j], index_col[j+2]) # ImgSeg: Após binarização - thresh
    ...
    for i in range(0,len(index_lin)-3,3):
         for j in range(0,len(index_col)-2,2):
            ...
            vLinha = index_lin[i:i+4]  #vais ter que adaptar, pois aqui já pego o vetor
            vColuna = index_col[j:j+3] #vais ter que adaptar, pois aqui já pego o vetor
            vLetras = [0,0,0,0,0,0]
            cnt = 0
            flag = 0
            for m in range(len(vColuna)-1):
                for n in range(len(vLinha)-1):
                    img = imageSeg[vLinha[n]:vLinha[n+1],vColuna[m]:vColuna[m+1]]
                
                    for k in range(len(centroids)):
                        if(centroids[k][0] >= vColuna[m] and centroids[k][0] <= vColuna[m+1] and  centroids[k][1] >= vLinha[n] and centroids[k][1] <= vLinha[n+1]):                    
                            vLetras[cnt] = 1            
                    cnt = cnt+1                
